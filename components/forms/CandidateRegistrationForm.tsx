...
export default function CandidateRegistrationForm() {
  const [form, setForm] = useState<CandidateFormData & { transaction_id: string }>(INITIAL);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg({
        type: "success",
        text: "Application submitted successfully. The IEC will review your submission and notify you via email.",
      });

      // Fire-and-forget — never crash the form
      try {
      ...
      setMsg({ type: "error", text: `Unexpected error: ${err instanceof Error ? err.message : String(err)}` });
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="grid gap-8">
      ...
      <Section title="Verification Documents" subtitle="All documents required. PDF, PNG, JPG accepted — max 5MB each.">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {/* PAYMENT — screenshot + transaction ID together */}
          <div className="grid gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-slate-500">
                ...
                The IEC will cross-verify this with bank records.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
        <p className="text-sm leading-7 text-amber-900">
          ...
          fraudulent documents, falsified payment references, or incomplete submissions will result
          in immediate disqualification and further IEC disciplinary action.
        </p>
      </div>
