...
export default function AdminCandidatesPage() {
  ...
  return (
    <AdminShell>
      ...
      {/* TABLE */}
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-xl bg-slate-200" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm">
          <p className="text-lg font-semibold text-slate-400">No candidates found</p>
          <p className="mt-2 text-sm text-slate-400">
            {candidates.length === 0 ? "No registrations have been submitted yet." : "Try adjusting your search or filters."}
          </p>
        </div>
      ) : (
                ...
                {filtered.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50">
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-xs text-slate-400">{fmtDate(c.submitted_at)}</td>
                      <td className="px-4 py-3">
                        <button onClick={() => { setSelected(c); setNotes(c.admin_notes ?? ""); setSaveMsg(""); }}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl">
                ...
                {[
                  ...
                  d.url
                    ? <a key={d.label} href={d.url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 hover:bg-blue-100 transition">
                        📎 {d.label}
                      </a>
                    : <div key={d.label} className="flex items-center gap-2 rounded-xl border border-red-100 bg-red-50 px-4 py-2.5 text-sm font-medium text-red-400">
                        ⚠ {d.label} — Missing
                      </div>
                ))}
              </div>
            ...
            {/* NOTES */}
            <div className="mb-6">
              <label className="mb-2 block text-sm font-medium text-slate-700">IEC Review Notes (visible to applicant)</label>
            ...
            {saveMsg && (
              <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-700">{saveMsg}</div>
            )}
